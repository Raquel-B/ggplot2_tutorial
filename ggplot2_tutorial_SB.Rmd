...
# Let's try a different family of fonts (see full list with font_families_google())
font_add_google("Amaranth")
```

Now, we can use those font families using: ...... `theme()`:

```{r use_theme_for_font_families} 
...
ggplot(chic, aes(x = date, y = temp)) +
  ...
  theme(plot.title = element_text(family = "Amaranth", hjust = .5, size = 10),
        plot.subtitle = element_text(family = "Amaranth", hjust = .5, size = 5))
```

```{r roboto_font}
# I don't get any error here 
#font_add_google("Roboto Condensed", "Roboto")
theme_set(theme_bw(base_size = 12, base_family = "Amaranth"))
```
...
ggplot(chic, aes(x = date, y = temp, color = season)) +
  geom_point() +
  labs(x = "Year", y = "Temperature (°F)") +
  theme(legend.title = element_text(family = "Amaranth",
                                    color = "chocolate",
                                    size = 14, face = "bold"))
```
...
ggplot(chic, aes(x = date, y = temp, color = season)) +
  ...
  theme(legend.key = element_rect(fill = "darkgoldenrod1"),
        legend.title = element_text(family = "Amaranth",
                                    color = "chocolate",
                                    size = 14, face = 2)) +
...
g + facet_wrap(year ~ season, nrow = 4, scales = "free_x") +
  theme(
    ...
    strip.text = element_textbox_highlight(
      family = "Amaranth", size = 12, face = "bold",
...
ggplot(chic, aes(x = date, y = temp)) +
  ...
  theme(
    ...
    strip.text = element_textbox_highlight(
      ...
      hi.labels = "Summer", hi.family = "Amaranth",
...
g + theme_bw(base_family = "Amaranth")
```
...
```{r base_size}
g + theme_bw(base_size = 30, base_family = "Amaranth")
```
...
theme_custom <- function (base_size = 12, base_family = "Amaranth") {
...
💡 You can only overwrite the defaults for all elements you want to change. Here I listed all so you can see that you can literally change everything!
